...
localidades_router = APIRouter(prefix='/localidades', tags=['Localidades'])
repo = LocalidadesRepo()


@localidades_router.get('/', response_model=List[LocalidadListModel], response_model_exclude_none=True)
def get_all(db: Session = Depends(db_instance.get_db)):
    result = repo.get_all(db)
    return result


@localidades_router.get('/{id}', response_model=LocalidadDetailModel)
...
@localidades_router.delete('/{id}', status_code=204, response_class=Response)
