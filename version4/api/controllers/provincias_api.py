...
from api.model.provincia_modelos import ProvinciaModel, ProvinciaSinIdModel
...
@provincias_router.get('/', response_model=List[ProvinciaModel], response_model_exclude_none=True)
def get_all(db: Session = Depends(db_instance.get_db)):
    return repo.get_all(db)


@provincias_router.get('/{id}', response_model=ProvinciaModel)
...
@provincias_router.post('/', response_model=ProvinciaModel, status_code=status.HTTP_201_CREATED)
def agregar(data: ProvinciaSinIdModel, db: Session = Depends(db_instance.get_db)):
    provincia = Provincia(**data.model_dump(exclude_unset=True))
    c = repo.agregar(db, provincia)
    return c


@provincias_router.put('/{id}', response_model=ProvinciaModel)
def editar(id: int, data: ProvinciaSinIdModel, db: Session = Depends(db_instance.get_db)):
    try:
